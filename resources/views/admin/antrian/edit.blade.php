...
<div class="card">
    ...
    <div class="card-body">
    <form action ="/antrian/{{ $antrians->id }}" method="post">
        @method('PUT')
        @csrf
        <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Nomor Antrian</label>
        <input type="text" class="form-control @error('no_antrian') is-invalid @enderror" id="no_antrian" name="no_antrian" value="{{old('no_antrian',$antrians->no_antrian)}}" readonly >
        @error('no_antrian')
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="pasien_id" aria-label="Default select example">
                ...
                @foreach($pasiens as $pasien)
                    @if (old('pasien_id',$antrians->pasien_id) == $pasien->id)
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tanggal_checkup') is-invalid @enderror" id="tanggal_checkup" name="tanggal_checkup" value="{{old('tanggal_checkup',$antrians->tanggal_checkup)}}" required autofocus>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="status_antrian" aria-label="Default select example">
                ...
                @if ($antrians['status_antrian']=="menunggu")
                    {{ 'selected' }}
                @endif>Menunggu</option>
                <option value="diperiksa" @if ($antrians['status_antrian']=="diperiksa"){{ 'selected' }}@endif>Selesai Di Periksa </option>
